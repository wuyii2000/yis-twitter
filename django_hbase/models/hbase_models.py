...
class HBaseModel:
    ...
    def deserialize_row_key(cls, row_key):
        ...
        # append ':' to the end, so that find(:) will find the last key
        row_key = row_key + ':'
        for key in cls.Meta.row_key:
            index = row_key.find(':')
            if index == -1:
    ...
    def batch_create(cls, batch_data):
        ...
        for data in batch_data:
            results.append(cls.create(batch, **data))
        batch.send()
        return results
    ...
    def get_table_name(cls):
        if not cls.Meta.table_name:
            raise NotImplementedError('Missing table_name in HBaseModel met class')
        if settings.TESTING:
